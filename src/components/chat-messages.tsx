  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
      {messages.map((message, index) => (
        <div
          // key={message.id}
          className={cn("flex gap-3 animate-slide-up", message.senderId === "user" ? "flex-row-reverse" : "flex-row")}
        ...
        >
          ...
          <div
            className={cn("flex flex-col gap-1 max-w-[70%]", message.senderId === "user" ? "items-end" : "items-start")}
          >
            <div
              className={cn(
            ...
            <span className="text-xs text-muted-foreground dark:text-slate-400 px-1">{message.createdAt}</span>
          </div>
        </div>
      ))}
